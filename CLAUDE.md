...
- **Snippet cut + robust stop**: once playback is *confirmed* started (`waitUntilPlaying`), a single `setTimeout(sec*1000)` (`snippetTimer`) calls `stopPlayback()`. `stopPlayback()` exists because the *first* play of a track is started over the REST/Connect channel (right after a device transfer) and the SDK's `pause()` can no-op on it until the SDK owns the device — the long-standing "first hint plays the whole song" bug. So it pauses via the SDK, then, when `startedViaRest` is true (set in `startCurrent`), forces an authoritative REST `me/player/pause`; for SDK-started plays it verifies via `getCurrentState()` and only REST-pauses if not actually paused. Keep the `[snipify]`-prefixed console logs until this is confirmed fixed in the wild. Don't reintroduce a polling loop to stop snippets — the old 250ms poll overshot short hints.
